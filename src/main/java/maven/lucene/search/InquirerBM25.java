	...
	public void queryIndexBM25() throws Exception
	{
		...
		for(int i = 0; i < querys.get(1).size(); i++) {
			...
			for(int j = 0; j < MAX_RESULTS; j++) {
				...
				writer.write(String.valueOf(i + 1) + " Q0 " + (scores.get(j).documentNumber + 1) + " " + (j+1) + " " + scores.get(j).score + " SimpleQueryBM25Scoring\n");
			}
		}            

		// close everything we used
		System.out.print("BM25 Querys are completed\n\n");
		ireader.close();
		directory.close();
		writer.close();
	}
