...
public class InquirerCustomScoring
{

	// the location of the search index
	private static String INDEX_DIRECTORY = "index";
	private static String QUERY_FILE = "corpus/cran.qry";
	private static String RESULTS_FILE = "results/SimpleQueryCustomScoring.txt";
	private static boolean appendToFile = false;

	// Limit the number of search results we get
	private static int MAX_RESULTS = 20;

	public void queryIndexCustomScoring() throws Exception{

		System.out.println("Inquiring with Custom Scoring");
		...
		//Seting custom scoring in the searcher
		isearcher.setSimilarity(new SimilarityTFIDF());
		...
		for(int i = 0; i < querys.get(1).size(); i++) {
			...
			for(int j = 0; j < MAX_RESULTS; j++) {
				...
				//Write to result file in correct fromat
				writer.write(String.valueOf(i + 1) + " Q0 " + (scores.get(j).documentNumber + 1) + " " + (j+1) + " " + scores.get(j).score + " SimpleQueryCustomScoring\n");
			}
		}            

		// close everything we used
		System.out.print("Custom Querys are completed\n\n");
		ireader.close();
		directory.close();
		writer.close();
	}
